<template>
    <div v-loading="logData === ''">
        ...
        <span>选择日志文件</span>
        <hr>
        <el-select v-model="select" style="width: 100%" placeholder="Select" size="large"
...
const logData = ref<string>(''), select = ref<string>('0'), logList = reactive<optionType[]>([]);
...
const updateContent = (option: number = parseInt(select.value)) => {
    ...
    getLogView(option).then(res => {
        logData.value = (<{ content: string }>res.data.data).content;
    });
}
</script>